...
for eps in itertools.product((0, 1), repeat=t):
    ...
    S = [m.NewBoolVar(f"S{eps}_{v}") for v in range(n)]
    cut_terms = []
    ...
    for (u, v), var in M.items():
        ...
        c = m.NewBoolVar(""); m.AddImplication(c, var); m.AddImplication(c, xr); m.AddBoolOr([var.Not(), xr.Not(), c]); cut_terms.append(c)
    bs = []
    for v in range(n):
...
if HINT:   # warm start from a dumped instance (killsets/climb json): map its circuits onto ours in order
    ...
    ref = d["ref"]; f = [ref.get(str(min(c)), 0) if isinstance(ref, dict) else ref[i] for i, (c, _) in enumerate(comps)]
    black = partition(comps, f); pidx = [i for i, (_, e) in enumerate(comps) if e]
    ...
    for (u, v), var in M.items(): m.AddHint(var, 1 if frozenset((u, v)) in mset else 0)
    for hv, v in vmap.items(): m.AddHint(x[v], 1 if hv in black else 0)
    for hv, v in vmap.items():
        for l in range(t + 1): m.AddHint(lab[v][l], 1 if labh[hv] == l else 0)
    hodd = [C for C in hc if len(C) % 2]
    for hC, zi in zip(hodd, d["zero"]): m.AddHint(zvar[vmap[hC[zi]]], 1)
...
while True:
    ...
    if st not in (cp_model.OPTIMAL, cp_model.FEASIBLE) or solver.ObjectiveValue() < 2 ** t: break
